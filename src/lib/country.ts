 ...
 */
export function formatCountryCode(entry: CountryCodeEntry | string): string {
    // Handle if entry is a simple string (old format)
    if (typeof entry === 'string') {
        return entry;
    }

    // Handle if entry is null or undefined
    if (!entry) {
        return 'Unknown Country';
    }

    const { numeric, alpha2, alpha3 } = entry;
    ...
    // Fallback
    return alpha2 || alpha3 || numeric || 'Unknown Country';
}